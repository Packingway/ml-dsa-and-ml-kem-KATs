...
def print_arrary(seeds):
    hex_string = seeds.hex()
    # 每16个字符为一组进行换行输出
    output = '\n'.join(hex_string[i:i+16] for i in range(0, len(hex_string), 16))
    # 打印结果
    print(output)
...
def print_A(data_3d):
    print('\n A:')
    # 将三维数组展平为一维数组
    data_1d = [item for sublist1 in data_3d for sublist2 in sublist1 for item in sublist2]

    # 将每个整数转换为24位的十六进制字符串（不足24位时补零）
    hex_data = [f"{x:06x}" for x in data_1d]

    # 每4个数据拼接在一起
    grouped_hex_data = [''.join(hex_data[i:i+4]) for i in range(0, 256, 4)]  # len(hex_data)

    # 输出结果
    for group in grouped_hex_data:
        print(group)

def print_S(data_2d):
    print('\n S:')
    # 将三维数组展平为一维数组
    data_1d = [item for sublist in data_2d for item in sublist]

    # 将每个整数转换为24位的十六进制字符串（不足24位时补零）
    hex_data = [f"{x:06x}" for x in data_1d]

    # 每4个数据拼接在一起
    grouped_hex_data = [''.join(hex_data[i:i+4]) for i in range(0, len(hex_data), 4)]  # len(hex_data)
...
def MLDSA_KeyGen_Internal(xi,variant=44):
    ...
    seeds=bytearray(128)
    seeds[0:32]=xi
    seeds[32:33]=converse.IntegerToBytes(k,1)  # 单个字节
    seeds[33:34]=converse.IntegerToBytes(l,1)  # 单个字节
    # print('input:',seeds.hex())
...
def MLDSA_Verify_Internal(pk,M_prime,sig,variant=44):
    ...
    # print_S(w1_prime)
    # print('\n\n\n\n\n\n\n\n')
    # for i in range(0, len(encode.w1Encode(gamma2,k,w1_prime)), 8):
    #     chunk = encode.w1Encode(gamma2,k,w1_prime)[i:i+8]  # 每次取 8 个字节
    #     hex_str = ''.join(f'{b:02X}' for b in chunk)  # 转换为 16 进制
    #     print(hex_str)
    # print(encode.w1Encode(gamma2,k,w1_prime))
...
if __name__ == '__main__':
    ...
    xi=bytearray.fromhex("5615a4565071763dbef4b9c704158bf07df8ef194bae8ded6cd3d62bf05e8266")  # 产生随机数作为种子
    pubkey,privkey=MLDSA_KeyGen_Internal(xi,variant=87)      # 使用种子生成公钥和私钥
    ...
    # sig=MLDSA_Sign_Internal(privkey,M,rnd,variant=44)
    # res=MLDSA_Verify_Internal(pubkey,M,sig,44)
    # if res:
    #     print("success")
